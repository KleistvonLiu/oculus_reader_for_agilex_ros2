...
"""
python oculus_reader/plot_t_matrix_log.py /home/kleist/Documents/Code/oculus_reader/oculus_reader/t_matrix_log.csv
...
def main():
    ...
    for pose_type in pose_types:
        type_rows = [r for r in rows if r.get("type", "unknown") == pose_type]
        if not type_rows:
            continue
        ...
        fig.suptitle(f"Pose: {pose_type}", y=0.98)
        fig.tight_layout()
